...
__all__ = [
	...
	#
	"DynamicMultiLayerPerceptronFromSpecifications",
	"DynamicMultiLayerPerceptronFromPretrainedModel",
]
...
def DynamicMultiLayerPerceptronFromSpecifications(
		input_layer,
...
def DynamicMultiLayerPerceptronFromPretrainedModel(input_layer, pretrained_network):
	neural_network = input_layer

	for layer in layers.get_all_layers(pretrained_network):
		if isinstance(layer, layers.DenseLayer):
			# print(neural_network, layer.num_units, layer.W, layer.b, layer.nonlinearity)
			neural_network = layers.DynamicDenseLayer(neural_network, layer.num_units, W=layer.W, b=layer.b,
			                                   nonlinearity=layer.nonlinearity)
		if isinstance(layer, layers.BernoulliDropoutLayer):
			# print(neural_network, layer.activation_probability)
			neural_network = layers.DynamicDropoutLayer(neural_network,
			                                             activation_probability=layer.activation_probability)
